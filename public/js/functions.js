...
return fecha < hoy; } function manejarEstadoFormulario(fechaPasada) { const
    formulario=document.getElementById("agregar-platillo-form-menu"); const
    botonesEliminar=document.querySelectorAll( "#tabla-platillos-menu button" ); const
    inputsCantidad=document.querySelectorAll(".cantidad-input"); const
    botonesGuardar=document.querySelectorAll(".guardar-cantidad"); if (formulario) {
    formulario.style.display=fechaPasada ? "none" : "block" ; } botonesEliminar.forEach((btn)=> {
    ...
    !nuevaCantidad ||
    isNaN(nuevaCantidad) ||
    parseInt(nuevaCantidad) <= 0 ) { alert("Ingresa una cantidad válida mayor a 0."); return; }
        console.log( "Guardando cantidad:" , platilloId, nuevaCantidad, fecha ); fetch(window.routes.actualizarCantidad,
        { method: "POST" , headers: { "Content-Type" : "application/json" , "X-CSRF-TOKEN" : window.csrfToken,
        Accept: "application/json" , }, body: JSON.stringify({ platillo_id: platilloId, cantidad: nuevaCantidad, fecha:
        fecha, }), }) .then((res)=> res.json())
        .then((data) => {
        if (data.success) {
        ...
        <div class="flex justify-between items-center">
            ...
            <div>
                <input type="number" name="cantidad_${item.id}" min="0" max="${item.cantidad_disponible}"
        ...
        <select id="editar-metodo-pago" class="mt-1 block w-full rounded border-gray-300 metodo_pago">
            ...
            <option value="transferencia" ${ data.metodo_pago === "transferencia" ? "selected" : "" }>Transferencia
            </option>
        </select>
        `;
