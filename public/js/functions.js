                ...
                data.pedidos && data.pedidos.length > 0
                    ? data.pedidos.map((pedido, index) => {
                          return [
                              ...
                              `<button class="text-blue-600 hover:text-blue-800 font-semibold text-xs" onclick="abrirModalEditarPedido(${pedido.id})">Editar</button>
                         <button class="text-red-600 hover:text-red-800 font-semibold text-xs ml-2" onclick="eliminarPedido(${pedido.id})">Eliminar</button>`,
                          ];
                      })
                    : [];
        ...
        .then((data) => {
            ...
            data.platillos.forEach((platillo, index) => {
                ...
                platilloHtml.innerHTML = `
                    ...
                    <input type="number" name="platillos[${index}][cantidad]" value="${platillo.cantidad}" min="0" max="${maxDisponible}"
            ...
            data.menu_dia.forEach((platillo) => {
                if (!idsExistentes.includes(platillo.id)) {
                    ...
                    platilloHtml.innerHTML = `
                        ...
                        <label class="block text-sm text-gray-600">Cantidad</label>
                        <input type="number" name="platillos[${indexNuevo}][cantidad]" value="0" min="0" max="${platillo.cantidad_disponible}"
                        class="mt-1 block w-full rounded border-gray-300" data-platillo-id="${platillo.id}" data-precio="${platillo.precio}">
